...
import { datavBoundaryUrl, datavSingleBoundaryUrl, rasterSourceInfo, townshipBoundaryUrl } from "./dataSources.js";
...
export const AMAP_RESIDENTIAL_LAYER_SOURCE = {
  id: "amap-web-residential-layer",
  label: "高德视野内住宅 POI",
...
export function initialPoiSearchState() {
  ...
  return {
    ...
    query: "",
    regionLabel: "全国",
    error: hasKey ? "" : "缺少 VITE_AMAP_WEB_KEY，无法调用高德 Web 服务搜索。",
    note: hasKey ? "先匹配当前行政区列表；未命中时调用高德 POI 搜索。" : "高德 POI 搜索未启用。",
  };
}
...
export function initialResidentialLayerState() {
  ...
  return {
    ...
    regionLabel: "全国",
    viewportLabel: "",
    error: hasKey ? "" : "缺少 VITE_AMAP_WEB_KEY，无法自动加载视野内小区层。",
    note: hasKey
      ? "缩放到街道尺度后，按当前视野自动加载住宅 POI 点位；只有高德返回 AOI 面时才显示真实小区面。"
      : "视野内小区层未启用。",
  };
}
...
export function sourceUrlForNode(node, collection = false) {
  ...
  if (node.level === "district" && node.feature?.properties?.provinceName && node.feature?.properties?.cityName) {
    return townshipBoundaryUrl({
      provinceName: node.feature.properties.provinceName,
      cityName: node.feature.properties.cityName,
      districtName: node.fullName || node.name,
    });
  }
