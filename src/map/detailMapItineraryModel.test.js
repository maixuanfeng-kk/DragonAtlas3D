...
test("buildDetailMapOverlayModel falls back to selected nodes before a plan exists", () => {
  const overlay = buildDetailMapOverlayModel({
    activeDay: 1,
    days: [],
    selectedNodes: [
      { id: "jianghan-road", name: "Jianghan Road", center: [114.291, 30.581] },
      { id: "yellow-crane-tower", name: "Yellow Crane Tower", center: [114.306, 30.547] },
    ],
  });

  assert.equal(overlay.markers.length, 2);
  assert.equal(overlay.markers[0].name, "Jianghan Road");
  assert.equal(overlay.markers[0].timeLabel, "Pending");
  assert.deepEqual(overlay.legs, []);
});
