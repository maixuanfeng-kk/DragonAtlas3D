...
function credentialNote(credentials) {
  if (credentials.usesFallbackKey) {
    return "Detail mode is currently falling back to VITE_AMAP_WEB_KEY. If map loading fails, add VITE_AMAP_JS_KEY and VITE_AMAP_JS_SECURITY_CODE.";
  }
  ...
  return "Amap JS map renders the city detail layer while the 3D terrain homepage remains the macro entry.";
}
...
export function AmapDetailView({ viewport, itineraryState, onBack, onSelectStop, onSelectLeg }) {
  ...
  useEffect(() => {
    ...
    async function mountMap() {
      ...
      try {
        ...
        if (!cancelled) {
          setLoadState({ status: "failed", error: error instanceof Error ? error.message : "Amap detail map failed to load." });
        }
      }
    }
  ...
  }, [itineraryState, onSelectLeg, onSelectStop]);
      ...
      <header className="detail-map-bar">
        <div className="detail-map-copy">
          ...
          <div className="detail-map-meta">
            <span>City itinerary detail</span>
      ...
      {loadState.status !== "ready" && (
        <div className="detail-map-mask" role="status" aria-live="polite">
          <div className="detail-map-mask-card">
            ...
            <p>{loadState.status === "pending" ? "Loading the Amap city detail map layer and syncing the current viewport." : loadState.error}</p>
          </div>
        </div>
      )}
    </section>
